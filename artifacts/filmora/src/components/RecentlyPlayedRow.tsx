...
const RecentlyPlayedRow = () => {
    ...
    useEffect(() => {
        ...
        window.addEventListener("historySettingChanged", loadRecent);
        return () => {
            window.removeEventListener("recentlyPlayedUpdated", loadRecent);
            window.removeEventListener("historySettingChanged", loadRecent);
        };
    }, []);
