...
import ShivMovieCard from "./ShivMovieCard";
...
const getHeaderConfig = (title: string) => {
    ...
    if (lowerTitle.includes("animated")) {
        return {
            ...
            badge: { text: "COSY PICKS", bg: "bg-pink-500/10 text-pink-400 border border-pink-500/20" },
...
const getSeeAllUrl = (title: string): string | null => {
    ...
    if (lower === "action blockbusters" || lower === "action movies") return "/categories/28?name=Action&type=movie";
    if (lower === "comedy hits" || lower === "comedy movies") return "/categories/35?name=Comedy&type=movie";
    if (lower === "horror movies" || lower === "chilling horror") return "/categories/27?name=Horror&type=movie";
    if (lower === "animation favorites" || lower === "animated wonders") return "/categories/16?name=Animation&type=movie";
    if (lower === "drama series" || lower === "gripping dramas" || lower === "drama movies") return "/categories/18?name=Drama&type=tv";
    if (lower === "sci-fi & fantasy") return "/categories/10765?name=Sci-Fi%20%26%20Fantasy&type=tv";
    if (lower === "adventure quests") return "/categories/12?name=Adventure&type=movie";
    if (lower === "crime thrillers") return "/categories/80?name=Crime&type=movie";
    if (lower === "mystery & suspense") return "/categories/9648?name=Mystery&type=movie";
    if (lower === "romantic getaways") return "/categories/10749?name=Romance&type=movie";
    if (lower === "reality obsessions") return "/categories/10764?name=Reality&type=tv";
    if (lower === "insightful documentaries") return "/categories/99?name=Documentary&type=movie";
    if (lower === "wild west tales") return "/categories/37?name=Western&type=movie";
    if (lower === "musical journeys") return "/categories/10402?name=Music&type=movie";
    if (lower === "historic wars") return "/categories/36?name=History&type=movie";
    if (lower === "action & adventure") return "/categories/10759?name=Action%20%26%20Adventure&type=tv";
    ...
    if (isShiv) {
        return (
            <motion.div
                ...
                className={cn("px-4 sm:px-8 md:px-12", className)}
            >
                {/* ── FTM Editorial Section Header ── */}
                <div className="mb-5 md:mb-7">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        />
                        <span
                            className="text-[10px] font-black tracking-[0.22em] uppercase flex-shrink-0"
                    ...
                    <div className="flex items-end justify-between">
                        ...
                        {resolvedSeeAllUrl && (
                            <Link
                                ...
                                className="text-[11px] font-black uppercase tracking-widest pb-0.5 border-b transition-colors duration-200"
                                style={{
                                    color: "var(--accent)",
