...
const Hero = ({ movies }: HeroProps) => {
    ...
    const [imageSize, setImageSize] = useState<'w780' | 'w1280'>('w1280');

    useEffect(() => {
        setImageSize(window.innerWidth < 768 ? 'w780' : 'w1280');
    }, []);
    ...
    return (
        <div data-hero="true" className="relative h-[75vh] sm:h-[80vh] md:h-[85vh] w-full overflow-hidden bg-black group">
            <TrailerModal
                ...
                >
                    ...
                    <div className="absolute inset-0">
                        <motion.img
                            ...
                            src={`https://image.tmdb.org/t/p/${imageSize}${heroMovies[current].backdrop_path}`}
                    ...
                    <div className="absolute inset-0 flex flex-col px-4 sm:px-6 md:px-12 lg:px-20 pt-24 pb-14 md:pb-32 justify-end">
                        <div className="max-w-3xl space-y-3 sm:space-y-4 md:space-y-6 z-10">
                            ...
                            {heroMovies[current].logos && (heroMovies[current].logos as any[]).length > 0 ? (
                                ...
                                >
                                    ...
                                    <img
                                        ...
                                        style={{ filter: "brightness(0) invert(1) drop-shadow(0 2px 16px rgba(0,0,0,0.9))" }}
                                    />
                                </motion.div>
                            ) : (
                                <motion.h2
                            ...
                            >
                                ...
                                {(heroMovies[current] as any).quality && (
                                    <>
                                        <span className="text-gray-600">•</span>
                                        <span className="px-1.5 py-0.5 rounded-full bg-white/10 text-[9px] sm:text-[10px] text-white border border-white/10 uppercase tracking-widest font-bold">
                                            {(heroMovies[current] as any).quality}
                                        </span>
                                    </>
                                )}
