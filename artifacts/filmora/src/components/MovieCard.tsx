...
const MovieCard = ({ movie, className, isFluid = false, isResume = false, onRemove, isFirst = false, isLast = false, posterMode = false }: MovieCardProps) => {
    ...
    return (
        ...
        >
            <div className={cn(
                "relative w-full overflow-hidden bg-white/5 border border-white/10 group-hover:border-white/30 transition-all duration-300",
                isPerson ? "aspect-[2/3]" : posterMode ? "aspect-[2/3]" : "aspect-video",
                isHovered && !posterMode && !isPerson ? "rounded-t-xl border-b-0 shadow-[0_20px_50px_rgba(0,0,0,0.8)]" : "rounded-xl"
            )}>
            ...
            {/* Title below poster — shown for posterMode and person cards */}
            {(posterMode || isPerson) && !isCompany && (
                <div className="mt-2 px-0.5 space-y-0.5">
                    <h3 className="text-xs sm:text-sm font-bold text-white leading-tight line-clamp-2 drop-shadow">
                        {movie.title || movie.name}
                    </h3>
                    {(movie.release_date || movie.first_air_date || (movie as any).vote_average) && (
                        <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-medium">
                            {(movie as any).vote_average > 0 && (
                                <span className="text-accent font-bold">★ {Number((movie as any).vote_average).toFixed(1)}</span>
                            )}
            ...
            {/* Detailed Hover Overlay - Netflix Style expanding below (landscape cards only) */}
            {isHovered && !posterMode && !isPerson && (
                <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.2 }}
                    className="absolute top-full left-0 right-0 z-30 movie-card-panel border-t-0 rounded-b-xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.8)] space-y-2 text-left"
                >
                    {isPerson ? (
                        <>
