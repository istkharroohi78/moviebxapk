...
import { Star, Calendar, Clock, ArrowLeft, User, Play, Youtube, X, Download, ChevronLeft, ChevronRight, Loader2, CheckCircle2, Send } from "lucide-react";
...
export default function WatchContainer({ type, id, tmdbData, initialSeason = 1, initialEpisode = 1, initialServer = 0, startPlaying = false }: WatchContainerProps) {
    ...
    const [telegramStreamUrl, setTelegramStreamUrl] = useState<string | null>(null);
    const [indexedOnlyMode, setIndexedOnlyMode] = useState(false);
    const [indexedCheckDone, setIndexedCheckDone] = useState(false);
    ...
    // Check indexed-only mode and look up Telegram file_id
    useEffect(() => {
        let cancelled = false;
        const check = async () => {
            try {
                const settingsRes = await fetch("/api/settings");
                if (!settingsRes.ok || cancelled) return;
                const settings = await settingsRes.json();
                if (!settings.indexedOnly) { setIndexedCheckDone(true); return; }
                setIndexedOnlyMode(true);
                const title = tmdbData?.title || tmdbData?.name || "";
                if (!title) { setIndexedCheckDone(true); return; }
                const findRes = await fetch(`/api/movies/find?title=${encodeURIComponent(title)}`);
                if (cancelled) return;
                if (findRes.ok) {
                    const movie = await findRes.json();
                    if (movie?.fileId) setTelegramFileId(movie.fileId);
                    if (movie?.streamUrl) setTelegramStreamUrl(movie.streamUrl);
                }
            } catch { /* ignore */ } finally {
                if (!cancelled) setIndexedCheckDone(true);
            }
        };
        check();
    ...
    const handleDownload = async (filePath: string) => {
        ...
        try {
            ...
            const proxyUrl = `${siteConfig.downloadProxy}?url=${encodeURIComponent(originalUrl)}`;
    ...
    return (
        ...
        >
            ...
            {/* Hero / Player Section */}
            <div data-mb-player className={`relative w-full z-40 transition-all duration-700 ${isPlaying ? "md:min-h-[85vh] shadow-[0_4px_40px_rgba(0,0,0,0.5)]" : "h-auto"}`}>
                ...
                {!isPlaying ? (
                    ...
                    /* Indexed-only mode: FtmPlyr streaming or Telegram deep link fallback */
                    <>
                        {!indexedCheckDone ? (
                            ...
                            /* FtmPlyr direct streaming player */
                            <div className="w-full bg-black">
                                <TelegramVideoPlayer
                                    src={telegramStreamUrl}
                                    poster={tmdbData?.backdrop_path ? `https://image.tmdb.org/t/p/w1280${tmdbData.backdrop_path}` : tmdbData?.poster_path ? `https://image.tmdb.org/t/p/w780${tmdbData.poster_path}` : undefined}
                                    title={tmdbData?.title ?? tmdbData?.name ?? ""}
                                    onBack={() => setIsPlaying(false)}
                                />
                            </div>
                        ) : telegramFileId ? (
                            /* Fallback: Telegram deep link when no stream URL */
                            <div className="relative w-full flex flex-col items-center justify-center bg-black min-h-[60vh] md:min-h-[85vh]">
                                {tmdbData?.backdrop_path && (
                                    <div
                                        className="absolute inset-0 bg-cover bg-center opacity-15"
                                        style={{ backgroundImage: `url(https://image.tmdb.org/t/p/w1280${tmdbData.backdrop_path})` }}
                                    />
                                )}
                                <div className="relative z-10 flex flex-col items-center gap-6 p-8 max-w-lg text-center">
                                    <div className="w-16 h-16 rounded-full bg-accent/20 border border-accent/40 flex items-center justify-center">
                                        <Send className="w-8 h-8 text-accent" />
                                    </div>
                                    <div>
                                        <h3 className="text-2xl font-black text-white mb-2">Get via Telegram</h3>
                                        <p className="text-gray-400 text-sm">Click below to get this movie instantly through our Telegram bot. The file will be sent directly to you.</p>
                                    </div>
                                    <a
                                        href={`https://t.me/${siteConfig.botUsername}?start=files_${telegramFileId}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center gap-3 px-8 py-4 bg-accent text-black font-black rounded-xl text-lg hover:bg-accent/90 transition-all hover:scale-105 shadow-lg shadow-accent/30"
                                    >
                            ...
                            <div className="relative w-full flex flex-col items-center justify-center bg-black min-h-[60vh] md:min-h-[85vh]">
                                <div className="relative z-10 flex flex-col items-center gap-6 p-8 max-w-lg text-center">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-black text-white mb-2">Not Available for Streaming</h3>
                                        <p className="text-gray-400 text-sm">This title hasn't been indexed yet. Check back later or request it via our Telegram bot.</p>
                                    </div>
                                    {siteConfig.botUsername && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center gap-2 px-6 py-3 border border-accent/40 text-accent font-bold rounded-lg hover:bg-accent/10 transition-all"
                                        >
