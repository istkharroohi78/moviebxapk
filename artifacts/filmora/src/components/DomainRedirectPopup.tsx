...
export default function DomainRedirectPopup() {
  ...
  useEffect(() => {
    ...
    if (!siteConfig.oldDomain) return;

    const hostname = window.location.hostname;
    if (hostname !== siteConfig.oldDomain) return;

    const destination = `${siteConfig.url}${window.location.pathname}${window.location.search}${window.location.hash}`;
