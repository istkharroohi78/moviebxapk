...
from tmdb_helper import tmdb_search as _tmdb_search
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            # Route all links through the filmora website /api/stream endpoint (proxied to bot).
            # The old bot-server URL (URL variable) is not accessible via the Replit public proxy.
            if WEBSITE_URL:
                lazy_stream = f"{WEBSITE_URL}/api/stream/{quote_plus(file_id)}"
                lazy_download = f"{WEBSITE_URL}/api/stream/{quote_plus(file_id)}"
            else:
...
async def auto_filter(client, msg, spoll=False):
    ...
    # Website watch/info buttons
    if WEBSITE_URL:
        _watch_url = f"{WEBSITE_URL}/search?q={quote_plus(search.strip())}"
        btn.append([
            InlineKeyboardButton("🌐 Watch Online", url=_watch_url),
            InlineKeyboardButton("ℹ️ Info", url=_watch_url),
        ])
...
async def ai_spell_check(chat_id, wrong_name):
    async def search_movie(wrong_name):
        search_results = await _tmdb_search(wrong_name)
